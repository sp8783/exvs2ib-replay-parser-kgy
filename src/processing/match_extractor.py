...
from src.ocr import ocr_on_matching_regions
...
class MatchExtractor:
    ...
    def _get_match_timestamp(self, frame_name):
        """
        フレーム名からタイムスタンプを計算する。
        """
        return calculate_timestamp(frame_name, self.frame_interval)
    
    def extract_match_results(self, screens, match_count):
        """
    ...
    def _extract_single_match(self, matching_frames, result_screen):
        """
        単一試合の情報を抽出する。
        """
        # OCR処理で最適なフレームを選択
    ...
    def _find_best_ocr_result(self, matching_frames):
        """
    ...
    def _get_result_info(self, result_type):
        """
        勝敗情報を取得する。
        """
